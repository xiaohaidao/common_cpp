...
int mm_client::connect(const char *address) {
  ...
  // connect
  handle_ = create_client(address, kClientSize, id_);
  pro->status = 1;
  std::error_code ec;
...
int mm_client::send(const char *data, size_t data_size) {
  ...
  if (is_server_) {
    write_mm += (kClientSize / 2);
  }
  uint32_t const &read_address = *(uint32_t *)write_mm;
  write_mm += sizeof(uint32_t);
  uint32_t &write_address = *(uint32_t *)write_mm;
  write_mm += sizeof(uint32_t);

  int const mm_size = kClientSize / 2 - sizeof(uint32_t) * 2;
  int remain_size = mm_size;

  // get remain
  if (read_address != write_address) {
    int rsize = (int)(write_address - read_address);
  ...
  // write data
  int const write_size = (std::min)(remain_size, (int)data_size);
  int const cover_edge = (int)(write_address + write_size) / mm_size;
  int const append_size = (int)(write_address + write_size) % mm_size;
  if (cover_edge > 0) {
    int const cover = write_size - append_size;
...
int mm_client::recv(char *data, size_t data_size) {
  ...
  if (!is_server_) {
    read_mm += (kClientSize / 2);
  }

  uint32_t &read_address = *(uint32_t *)read_mm;
  read_mm += sizeof(uint32_t);
  uint32_t const &write_address = *(uint32_t *)read_mm;
  read_mm += sizeof(uint32_t);

  uint32_t const mm_size = kClientSize / 2 - sizeof(uint32_t) * 2;
  int remain_size = 0;

  // get read remain
  if (read_address != write_address) {
    int rsize = (int)(write_address - read_address);
    rsize += (int)(rsize < 0 ? mm_size : 0);
    remain_size = rsize;
  }

  // read data
  int const read_size = (std::min)(remain_size, (int)data_size);
  int const cover_edge = (int)((read_address + read_size) / mm_size);
  int const append_size = (int)((read_address + read_size) % mm_size);
  if (cover_edge > 0) {
    int const cover = read_size - append_size;
